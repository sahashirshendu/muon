...
wm title . "Muon Physics GUI V1.2"
...
button .f2.1.1.start -width 20 -text "Start" -command {
    ...
    set message 0
    if { [catch { set serial [open $port r] }] == 1  } { 
        tk_messageBox -message "Program can not open $port. Check if it is used or does not exist."
        set message 1
    } 
    ...
    if { $message == 0 && [ expr  $number < 200 ] == 1 } {
        tk_messageBox -message "Minimum number of records to load is 200"
	set number 200
        set message 1
    }
    ...
    if { $message == 0 && [catch {clock scan $time1 }] == 1 } {
        tk_messageBox -message "From(Y-M-D): $time1 is not acceptable, set to default."
	set time1 [clock format [expr [clock scan now] - 436800] -format %Y-%m-%d]
        set message 1
    }
    ...
    if { $message == 0 && [catch {clock scan $time2 }] == 1 } {
        tk_messageBox -message "To(Y-M-D): $time2 is not acceptable, set to default."
	set time2 [clock format [clock scan now] -format %Y-%m-%d]
        set message 1
    }
    ...
    if { $message == 0 && [expr  [clock scan $time1] > [clock scan $time2] ] } {
        tk_messageBox -message "From: $time1 must eariler than To:$time2, set to default"
	set time1 [clock format [expr [clock scan now] - 436800] -format %Y-%m-%d]
	set time2 [clock format [clock scan now] -format %Y-%m-%d]
	...
	fconfigure $serial -blocking 0 -translation auto -buffering full -buffersize 64k
#
...
proc muonconfig { } {
     ...
     global port readold timescale bins number time1 time2 Font3 numdate serial
     set readold 0
     set numdate 0
...
#    port select
#
     labelframe .config.1.1 -text "Select port"  -font $Font3
...
#
     ...
     labelframe .config.1.2 -text "Decay time histogram scale"  -font $Font3
     foreach value { 3 6 12 20  } {
             radiobutton .config.1.2.$value -text "$value \u03bc S" -variable timescale -value $value -font $Font3
...
#
     ...
     labelframe .config.1.3 -text "Select bin number" -font $Font3
...
#
     ...
     checkbutton .config.2.2.numdate -text "Or specify the time window" -variable numdate -command\
...
proc muonstart { a } {
     global port timescale bins serial fpmuon STOP YAXIS y counts width K  B logl sigma kx2
...
proc readport { } {
    ...
    global serial fpmuon nmuon dmuon STOP thNumber READ
    ...
    after 10 { readport ; set READ 0 }
}


proc generate { } {
        global serial fpmuon nmuon dmuon STOP thNumber
	if { $STOP ==  0 } {
	   if { [expr rand()] > 0.90 } {
		set Rand [expr -log([expr rand()])*2120 ]
		set Rand [format %5.0f $Rand]
		incr nmuon
		incr dmuon
		incr  thNumber
                muonstart $Rand
                puts $fpmuon "$Rand [clock seconds]"

	}   else {
        incr nmuon
        incr thNumber
        puts $fpmuon "40000 [clock seconds]"
	}
	}
...
proc decayrate { } {
    global dmuon drate lastdmuon  fpmuon STIME
...
proc saveornot { } {
    ...
    label .topsave.f1.text -text "Save data from this run?"
